...
uint8_t ledDelayCount = 0;

const char* Version = "v1.0.1";
const char* Hardware = "STM32F042-CANable_0.2";
...
void CanBusActivityStatus(uint8_t status){
	if(status) {
		if(ledDelayCount > 15){
			PortB_On(LED_Blue_Pin); // green led is on PORTD. This is defined in main.h generated by CUBE-MX
		}
		ledDelayCount += 1;
	} else {
		if(ledDelayCount > 17) {
